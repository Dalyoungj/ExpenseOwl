...
// filterExpensesByCategories filters expenses to only include specified categories
func filterExpensesByCategories(expenses []storage.Expense, categories []string) []storage.Expense {
	if len(categories) == 0 {
		return expenses
	}
